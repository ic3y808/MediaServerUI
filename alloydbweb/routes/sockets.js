...
module.exports.loadSettings = function (key, callback) {
  try {
    var settingsResult = module.exports.db.prepare("SELECT * from Settings WHERE settings_key=?").get(key);
...
module.exports.saveSettings = function (key, value, callback) {
  try {
    const stmt = module.exports.db.prepare("INSERT OR REPLACE INTO Settings (settings_key, settings_value) VALUES (?, ?) ON CONFLICT(settings_key) DO UPDATE SET settings_value=?");
...
module.exports.socketConnect = function (socket, db) {
