...
function MainCtrl($q, $window, $document, $timeout, $templateRequest, $compile, $exceptionHandler) {
  ...
  function _updateTarget() {
    ...
    if (that.selectedItem.genre) { _setTargetValue(that.selectedItem.genre); }
    if (that.selectedItem.album) { _setTargetValue(that.selectedItem.album); }
    if (that.selectedItem.artist) { _setTargetValue(that.selectedItem.artist); }
