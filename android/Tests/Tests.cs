	...
	namespace Tests
	{
		...
		public class Tests
		{
			...
			private void SelectMenuItem(string item)
			{
				...
				AppResult[] soundcloudButton = app.WaitForElement(c => c.Marked("design_menu_item_text"));
				Assert.IsTrue(soundcloudButton.Any());

				foreach (var element in soundcloudButton)
				{
			...
			public void BasicOperationTests()
			{
				...
				SelectMenuItem("Soundcloud");
				CheckItemsLoaded("stream_list", "card_layout");
				SelectTab("Favorites");
